...
function PreviewCard({ preview, cardRef, onOpenFile, supabase }) {
  ...
  return (
    <div
      ...
      className="flex-shrink-0 w-40 md:w-48 snap-start bg-surface-container-lowest border border-outline-variant rounded-lg overflow-hidden group cursor-pointer hover:shadow-md transition-all"
    >
...
export default function QuickPreview({ previews, title = 'Preview Update Terkini', slider = false, onOpenFile, supabase, mobileLimit, showViewAll = true }) {
  ...
  // Filter previews for mobile - use mobileLimit prop if provided, otherwise show all
  const displayPreviews = isMobile && mobileLimit ? previews.slice(0, mobileLimit) : previews;
